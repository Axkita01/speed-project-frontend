...
export default function Game({navigation, route}) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Button 
      title = 'exit game' 
      onPress = {() => {socket.current.disconnect();
      navigation.navigate('home')}}/>
      {winningString(win)}
      <FlatList style = {{marginBottom: '.5vw'}}
      data = {oppLength}
      ...
      onPress = {
        () => { if (selected.current !== null && win == null &&
                    (hand[selected.current]['number'] == parseInt(deckTops[0]['number']) - 1 ||
                     ...
                     (deckTops[0]['number'] == '13' && hand[selected.current]['number'] == '1') ||
                     (deckTops[0]['number'] == '1' && hand[selected.current]['number'] == '13') 
                     )
                     ) {
      ...
      onPress = {
        () => { if (selected.current !== null && win == null &&
              ...
              (deckTops[1]['number'] == '13' && hand[selected.current]['number'] == '1') ||
              (deckTops[1]['number'] == '1' && hand[selected.current]['number'] == '13')
              )
              ) {
                if (hand.length === 1 && deck.length === 0) {
                  socket.current.emit('winner')
